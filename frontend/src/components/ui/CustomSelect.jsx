        ...
        <div className="relative">
          <Listbox.Button 
            className={`
              ...
              py-2.5 pl-4 pr-10 text-left text-sm text-100 transition-all duration-300
          ...
          >
            <Listbox.Options className="absolute z-50 mt-2 max-h-60 w-full overflow-auto rounded-2xl glass py-1.5 shadow-2xl focus:outline-none scrollbar-thin">
              {options.length === 0 ? (
                <div className="py-4 text-center text-500 text-sm">Aucune option disponible</div>
              ) : (
                options.map((option, optionIdx) => (
                  <Listbox.Option
                    key={optionIdx}
                    className={({ active, selected }) =>
                      `relative cursor-default select-none py-2.5 pl-10 pr-4 mx-1.5 rounded-lg transition-all duration-200 ${
